...
> (**MongoDB**) **se reemplaza por [Supabase](0002-adoptar-supabase-persistencia.md)**
> (Postgres gestionado + Auth + Storage). El sitio tendrá backend vía Supabase
> (BaaS), no servidor propio; la **integración es futura** (todavía no está en
> el repo: hoy corre 100% frontend). Ver [ADR-0002](0002-adoptar-supabase-persistencia.md).
> El resto del stack sigue vigente, con estas precisiones respecto del texto
> original: Tailwind v4 se usa **CSS-first** (sin `tailwind.config.js`; tema en
